...
//Code Here
let evens = mixedNumbers.filter(num => num % 2===0);
...
let orderTotals = orders.map((order) => order.price + order.tax)
...
let bobsTotal = purchases.filter((purchases) => purchases.owner === 'Bob').reduce((acc, curr) => acc + curr.price, 0)
